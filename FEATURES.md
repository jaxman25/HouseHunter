...
| Global pause (`notificationsPaused`) — scheduled function skips the user entirely when true | `functions/src/savedSearchNotifications.ts`, `functions/src/index.ts` |
| Per-type toggles (`notificationPrefs` on user doc) — each boolean controls whether that notification type is delivered; `system` is always true and not user-toggleable | `src/screens/settings/NotificationPreferencesScreen.tsx` |
| NotificationPreferencesScreen — manages global pause, per-type toggles, and per-saved-search frequency + isActive | `src/screens/settings/NotificationPreferencesScreen.tsx` |
| Legacy users without `notificationPrefs` default to all-on | `functions/src/savedSearchNotifications.ts`, `functions/src/index.ts` |
